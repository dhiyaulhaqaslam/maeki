...
function Welcome() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (kapalRef.current) kapalRef.current.style.transform = `translateY(${scrollY * 0.3}px)`;
  ...
  return (
    <>
      ...
      <section className="relative h-screen overflow-hidden bg-black">
        ...
        <div className="relative flex h-full flex-col items-center justify-center text-white">
          ...
          {awanList.map((awan, i) => (
            <img
              key={i}
              ref={(el) => { awanRefs.current[i] = el; }} // penting: kurung kurawal agar callback ref return void
              src={awan.src}
              alt={`Awan ${i + 1}`}
              className={`absolute opacity-90 select-none ${awan.style}`}
            />
          ))}
        </div>
      </section>
      ...
      {/* Section berikutnya */}
      <section className="flex h-screen items-center justify-center bg-gradient-to-b from-black to-neutral-900 text-white">
